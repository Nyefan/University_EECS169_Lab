...
public class AddSubtract {
	...
	public static void main(String[] args) { 
		...
		do {
			int x = getInput("1st");
			int y = getInput("2nd");
			addSubtractIntegers(x,y); // Outputs the sum and difference of x and y and notifies the user that the operation has been handled inside the method;
			
			//Outputs the sum and difference of x and y, notifying the user that the operations have been handled seperately from within main;
	...
	public static void addSubtractIntegers(int x,int y) { //Prints the values of (x + y) and (x - y) with appropriate notifiers;
		int sum = addIntegers(x,y);
		int diff = subtractIntegers(x,y);
		System.out.println("You are inside the Method called addSubtractIntegers, where Sum=" + sum + " and Difference=" + diff);
	}
	...
	public static boolean checkShouldLoop() { //Asks the user if the program should continue to loop and returns the appropriate boolean representation
